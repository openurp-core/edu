...
table.printTableStyle td{
 ...
 [#list lessonGradeReports as report]
    ...
    [#assign schoolName]${report.grades.std.project.school.name}[/#assign]
    [#assign stdTypeName = (report.grades.std.type1.name)!"" /]
   ...
   <div align='center'><h3>${schoolName}${teachTask.calendar.studentType.name}课程成绩登记表</h3></div>
   <div align='center'>${teachTask.semester.schoolYear}年度 <#if teachTask.semester.name='1'>第一学期<#elseif teachTask.semester.name='2'>第二学期<#else>${teachTask.semester.name}</#if></div>
   <table width='100%' align='center' border='0' style="font-size:13px" >
    <tr>
      ...
      <td align='left'>课程类别:${(teachTask.course.courseType)!}</td>
    </tr>
    <tr>
      <td>课程序号:${teachTask.no!}</td>
      <td>主讲教师:${(teachTask.teacher.name)!}</td>
      <td align='left'>授课院系:${(teachTask.teachDepart.name)!}</td>
    </tr> 
    <tr>
        <td colspan="3">
        <#list report.gradeTypes as gradeType>
        <#if teachTask.gradeState.getPercent(gradeType)?exists><@i18nName gradeType/>${teachTask.gradeState.getPercent(gradeType)?string.percent}&nbsp;
        </#if>
        </#list>
        </td>
    </tr>
