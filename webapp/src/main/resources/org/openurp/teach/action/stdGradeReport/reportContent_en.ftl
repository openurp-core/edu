 ...
 <#list stdGradeReports as report>
     ...
     <#assign schoolName><@i18nName systemConfig.school/></#assign>
 	 ...
 	 <#if !isSingle?exists>
 	 	...
 	 	<#else>
 	 	  ...
 	 	  <td width="${tableTitleColumnLen}%">&nbsp;&nbsp;Major：<#if aspectName?exists><#if aspectName?starts_with(majorName)>${aspectName}<#else>${majorName}<#if aspectName?length !=0>(${aspectName})</#if></#if><#else>${majorName}</#if></td>
 	 </#if>
     ...
     <table ${style}  class="listTable">
 	 ...
 	 <table ${style}>
          ...
          <#--上海金融学院：不管以哪个成绩大卡打印，总平均绩点均为最好绩点。-->
